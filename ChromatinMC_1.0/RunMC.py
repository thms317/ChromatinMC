...
def MultiMC(k):
    ...
    '''
    force1 = np.arange(4,6)
    force2 = np.arange(3,6)
    force3 = np.linspace(11,8,12)
    force4 = np.linspace(8,11,12)


    
    if k == 1:
        forces = force1
        dna, dyads, nucl = FMC.create_dna(697, 1, 197, unwrap = 0, nuc_file = '1KX5.3DNA')
        nucl.fixed_g = 3*kBT
        g = 3
    if k == 2:
        forces = force2
    ...
    if k == 3:
        forces = force3
        dna, dyads, nucl = FMC.create_dna(n_bp, n_nuc, NRL, unwrap = -70, nuc_file = '1KX5.3DNA')
    if k == 4:
        forces = force4[9:]
        dna, dyads, nucl = FMC.create_dna(697, 1, 197, unwrap = 0, nuc_file = '1KX5.3DNA')
...
#    freeze_support()
#    pool = Pool(4)
#    forces = np.linspace(0,5, 28)
#    
#
#    
#    runtime1 = time.time()
#    print runtime1
#    result1 = pool.map(SingleMC, forces)
