...
def read_xlsx(filename, dataset, pars=None):
...
def main():
    filename = 'E:\\users\\noort\\data\\20180321\\2nucs_014.dat'
    plot_step_params(filename, 0, save=True)
    x0 = np.load('C:\\Python27\\Lib\\site-packages\\helixmc\\data\\DNA_gau.npy')[0]
    sigma = np.load('C:\\Python27\\Lib\\site-packages\\helixmc\\data\\DNA_gau.npy')[1:]
    print(x0)
    print(sigma)
    k = 1/sigma**2
    print(0.5*x0*np.dot(k,x0)/kT)
    return
